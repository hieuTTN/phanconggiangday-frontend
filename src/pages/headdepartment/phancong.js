...
const TBMPhanCong = ()=>{
    ...
    const getPhanCong = async() =>{
        ...
        var response = await getMethod('/api/giang-vien/head-department/giang-vien-so-tiet?idnamhoc='+selectNamHoc.id+'&size='+size+'&page='+0)
        var result = await response.json();
        console.log(result);
        setItems(result.content)
        setpageCount(result.totalPages)
        url = '/api/giang-vien/head-department/giang-vien-so-tiet?idnamhoc='+selectNamHoc.id+'&size='+size+'&page='
    };
